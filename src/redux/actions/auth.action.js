import { LOG_IN_SUCCESS, LOG_IN_FAILED, LOG_OUT } from "../actionTypes";
import AsyncStorage from "@react-native-community/async-storage";

export const signIn = (username, password) => async dispatch => {
    const res = { token: { accessToken: "adsfasdfads", refreshToken: "asdfasdfa" }, info: { username } };
    try {
        //res = api call
        await AsyncStorage.setItem("billling_app_user_token", JSON.stringify(res.token));
        dispatch({type: LOG_IN_SUCCESS, payload: res});
    } catch (error) {
        dispatch({type: LOG_IN_FAILED, payload: { errMsg: "failed" }});
        throw error;
    }
}

export const isSignedIn = () => async dispatch => {
    try {
        //res = api call
        const token = await AsyncStorage.getItem("billling_app_user_token");
        if(!token) throw new Error("no token defined");
        dispatch({type: LOG_IN_SUCCESS, payload: { token: JSON.parse(token) }});
    } catch (error) {
        dispatch({type: LOG_IN_FAILED, payload: { errMsg: "failed" }});
        throw error;
    }
}

export const signOut = () => async dispatch => {
    try {
        //res = api call
        await AsyncStorage.removeItem("billling_app_user_token");
        dispatch({type: LOG_OUT});
    } catch (error) {
        dispatch({type: LOG_OUT});
        throw error;
    }
}
